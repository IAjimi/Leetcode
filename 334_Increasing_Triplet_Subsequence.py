		...
		if len(nums) < 3:
			...
			while i < j < k <= len(nums) - 1:
				if nums[i] < nums[j] < nums[k]:
					...
					j = k if nums[k] > nums[i] else j  # move j to this intermediary value (nums[i] < nums[k] <= nums[j])
					k += 1
...
if __name__ == '__main__':
	...
	assert Solution().increasingTriplet([2, 1, 9, 0, 4, 6])  # remove that solution - only 0 4 6
	...
	assert Solution().increasingTriplet([2, 2, 2, 0, 4, 6])  # try with duplicated numbers
